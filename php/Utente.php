...
try {
	...
	if((isset($_SESSION['userType'])) && ($id==$_SESSION['id'])){
		...
		$modificaBio='<form action="utils/operations.php" method="post" name="modificaBioform"><fieldset><label for="bioTesto"></label><textarea rows="5" cols="50" id="bioTesto" name="bioTesto" >'.getUserBio($id,$connessione).'</textarea><label for="modificaBio"></label><input id="modificaBio" class="button" name="modificaBio" value="Modifica Bio" type="submit"/></fieldset></form>';
		if($_SESSION['userType']=="1"){
			// button per creare nuovo treno
			$creaPagina='<form action="CreaTreno.php" method="post" class="utenteAction" name="addTrenoform"><fieldset><label class="hidden" for="creaTreno"></label><input id="creaTreno" class="button" name="creaTreno" type="submit" value="Crea Treno"/></fieldset></form>';
		}
	}
